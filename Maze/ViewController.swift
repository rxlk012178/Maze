...
class ViewController: UIViewController {
    ...
    func startAcceleromater() {
        ...
        let handler: CMAccelerometerHandler = {(CMAccelerometerData: CMAccelerometerData?, error: Error?) ->
            ...
            // 画面上からプレイヤーがはみ出しそうだったら、posY/posXを修正
            ...
            for wallRect in self.wallRectArray {
                if wallRect.intersects(self.playerView.frame){
                    self.gameCheck(result: "gameover", messeage: "壁に当たりました")
                    return
                }
            }
            
            if self.goalView.frame.intersects(self.playerView.frame) {
                self.gameCheck(result: "clear", messeage: "クリアしました！")
                return
            }
    ...
    func gameCheck(result: String, messeage: String) {
        // 加速度を止める
        if playerMotionManager.isAccelerometerActive {
            playerMotionManager.stopAccelerometerUpdates()
        }
        
        let gameCheckAlert: UIAlertController = UIAlertController(title: result, message: messeage, preferredStyle: .alert)
